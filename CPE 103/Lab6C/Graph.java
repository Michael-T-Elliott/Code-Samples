...
public class Graph implements Serializable
{
   ...
   public int Kruskal()
   {
      Heap newHeap = new Heap(numVertices, 0);
      int totalWeight = 0;

      for(int i = 0; i < numVertices; i++){
         for(int j = i; j < numVertices; j++){
            if(adj[i][j] > 0){
               Edge temp = new Edge(i, j, adj[i][j]);
               newHeap.insert(temp);
            }
         }
      }
      ...
      for(int i = 0; i < size; i++){
          ...
          Edge newEdge = (Edge)newHeap.remove();
          int A = newEdge.nodeA;
          int B = newEdge.nodeB;
          int weight = newEdge.weight;
          DisjointSets.Set SetA = DS.find(A);
          DisjointSets.Set SetB = DS.find(B);
