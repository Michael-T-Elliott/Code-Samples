...
public class Maze extends Applet {
   ...
   public boolean makeMaze(String fileName)
   {
      try
      {
         ...
         numVertex = (loc[rows-1][cols-1]+1);
         String eol = System.getProperty("line.separator");
         System.out.println("Vertices: " + numVertex);
         
         try (Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("AdjMatrix.txt"), "utf-8"))) {
        	 writer.write(numVertex + " UNDIRECTED " + "UNWEIGHTED ");
             for(int i=0; i<rows; i++)
             {
                for(int j=0; j<cols; j++)
                {  
                	if(j != cols-1 && maze[i][j+1] == 0 && maze[i][j] == 0){
                		writer.write(eol + loc[i][j] + " " + loc[i][j+1]);
                	}
                	
                	if(i != rows-1 && maze[i+1][j] == 0  && maze[i][j] == 0){
                		writer.write(eol + loc[i][j] + " " + loc[i+1][j]);
                	}
                }
             }
         }
         
         Graph mazeGraph = new Graph("AdjMatrix.txt");
         mazeGraph.print();
